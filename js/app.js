...
function buscarClima(e) {
    ...
    if(ciudad === '' || pais === '') {
        // Mensaje de error
        mostrarError("Por favor, rellena el formulario para conocer tu clima")
...
function consultarAPI(ciudad, pais ) {
        // Consultar la API y mostrar el clima del lugar

    // leer la url  y agregar el API key
    const appId = '5214ec0c97d1ade034b47bd9dfd8ee26';
    ...
    fetch(url)
      ...
      .then(datos => {
        ...
        limpiarHTML();
        if(datos.cod === "404") {
          mostrarError("Lugar no disponible")
        } else {
          mostrarClima(datos)
        }
      })
...
function mostrarClima(datos) {
  ...
  nombreCiudad.innerHTML = `Estado meteorológico de: ${name}`;
